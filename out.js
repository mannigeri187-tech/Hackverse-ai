...
// api/shared/supabase.js
function sanitizeEnvString(val) {
  if (!val) return "";
...
var supabaseUrl = sanitizeEnvString(rawUrl);
var supabaseKey = sanitizeEnvString(rawKey);
...
async function authenticateServerRequest(req) {
  ...
  try {
    ...
    if (parts.length === 3) {
      ...
      if (payload && payload.sub && (!payload.exp || payload.exp * 1e3 > nowMs)) {
        const user = {
          id: payload.sub,
          email: payload.email || "",
        ...
        const ttlMs = payload.exp ? Math.min(6e4, payload.exp * 1e3 - nowMs) : 6e4;
  ...
  try {
    ...
    tokenCache.set(token, { user, expiresAt: nowMs + 6e4 });
    return { user, error: null };
  } catch (err) {
