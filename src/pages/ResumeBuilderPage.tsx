...
export default function ResumeBuilderPage() {
  ...
  const { printRef, exportPdf, isExporting, exportError } = useResumePdfExporter(resumeData);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="h-[calc(100vh-200px)] overflow-y-auto sticky top-4 flex flex-col gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div className="flex gap-1.5 bg-slate-100 p-1 rounded-lg">
                  {(['ats-pro', 'modern', 'tech'] as const).map(t => (
                    <button
                      key={t}
                      onClick={() => setTheme(t)}
                      className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all capitalize ${theme === t ? 'bg-white text-primary-700 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                    >
                      {t === 'ats-pro' ? 'ATS Pro' : t}
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-2">
                {exportError && <span className="text-xs text-red-500 mr-2">{exportError}</span>}
                <button
                  onClick={exportPdf}
                  disabled={isExporting}
                  className="flex items-center gap-2 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-sm font-bold rounded-lg transition-colors disabled:opacity-70 shrink-0"
                >
